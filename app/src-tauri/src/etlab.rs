...
//! an etlab page means already live and where they can be unit-tested without
//! a running portal.
//!
//! It has to be native rather than `fetch` from the webview for three reasons:
//! a college portal will not send CORS headers to an app origin, the session is
//! a cookie the webview would refuse to expose to JavaScript, and login is a
//! redirect chain that has to be followed with the jar intact.
...
#[tauri::command]
pub fn etlab_start(state: State<'_, Etlab>, base: String) -> Result<(), String> {
...
#[tauri::command]
pub fn etlab_reset(state: State<'_, Etlab>) -> Result<(), String> {
    *state.inner.lock().map_err(|_| "session lock poisoned")? = None;
    Ok(())
}

#[tauri::command]
pub fn etlab_active(state: State<'_, Etlab>) -> bool {
    state.inner.lock().map(|s| s.is_some()).unwrap_or(false)
}

fn parts(state: &State<'_, Etlab>) -> Result<(reqwest::Client, String), String> {
...
#[tauri::command]
pub async fn etlab_get(state: State<'_, Etlab>, path: String) -> Result<Fetched, String> {
